...
test('light theme loads only active shared theme images and defers footer social icons', async ({ page }) => {
  const requestUrls = [];
  ...
  await expect(themeToggle).toHaveAttribute('aria-label', 'Przełącz na tryb ciemny');
  ...
  await expect(themeToggle).toHaveAttribute('aria-label', 'Przełącz na tryb jasny');
  await expect.poll(() => requestedAsset(requestUrls, 'logo-translogix-dark-')).toBe(true);
  await expect.poll(() => requestedAsset(requestUrls, 'moon-')).toBe(true);
});

test('stored dark theme loads only dark shared theme images initially', async ({ page }) => {
  const requestUrls = [];
  page.on('request', (request) => requestUrls.push(request.url()));
  ...
  await expect(page.locator('html')).toHaveClass(/theme-dark/);
  await expect(page.locator('header .brand__logo')).toHaveAttribute('src', /logo-translogix-dark/);
  await expect(page.locator('[data-theme-image="toggle"]')).toHaveAttribute('src', /moon-/);
...
test('system dark preference is used when no theme is stored', async ({ page }) => {
  await grantSiteConsent(page);
  ...
  expect(await page.evaluate(() => window.localStorage.getItem('translogix-theme'))).toBeNull();
});

test('built shared header and footer retain their no-JavaScript image baseline', async ({ browser }) => {
  ...
  await expect(page.locator('header .brand noscript img')).toHaveAttribute('src', /logo-translogix-light/);
  await expect(page.locator('.theme-toggle noscript img')).toHaveAttribute('src', /sun-/);
  await expect(page.locator('.brand--footer noscript img')).toHaveAttribute('src', /logo-translogix-light/);
  const scriptedImages = page.locator('[data-theme-image], [data-deferred-image]');
