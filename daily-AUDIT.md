...
- `npm run release-check` returned exit code 1. `qa:html`, `qa:jsonld` (11 blocks), `qa:links` (12 files), `qa:a11y` (12/12 URLs with zero errors), `assets:verify`, `qa:budget` and `qa:package` passed. `test:e2e` returned 12/13 passed because `aria-current.spec.js` attempted to import the source-only `/assets/js/aria-current.js` path while Playwright served the Vite-built `dist/` package; one focused retry reproduced the same failure, recorded as `PH5-08`. The first Pa11y server start also emitted a transient `EADDRINUSE`, but collection completed and the aggregate advanced normally.
...
- PH6-01 is complete as a verification task: every current release gate and Lighthouse URL has a recorded result, while the deterministic unresolved E2E, dependency, Lighthouse-contract, semantics and image-delivery issues remain separate implementation work.
